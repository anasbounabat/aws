...
onMounted(async () => {
  try {
    await auth.loadMe()
    await $api(`/invitations/${id.value}/accept`, { method: 'POST' })
    state.value = 'done'
...
<template>
  <div class="text-slate-200">
    <div v-if="state === 'loading'">Accepting invitation…</div>
    <div v-else-if="state === 'done'">Invitation accepted. Redirecting…</div>
